...
$(cat "$OLD_TEXT")"
    ...
    if [ -n "$SONNET_RESULT" ]; then
        ...
        while IFS= read -r line || [ -n "$line" ]; do
            ...
            elif [[ "$line" =~ ^\*\*Jarvis:\*\* ]]; then
                SUMMARY=$(echo "$line" | sed "s/^\*\*Jarvis:\*\* //" | head -c 120 | sed "s/[[:space:]]*$//")
